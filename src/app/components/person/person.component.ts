...
export class PersonComponent implements OnInit {
  ...
  getPersonsById(path) {
    ...
    setTimeout(() => {
      this.loading = false;
      if (path === 'id') {
        this.filteredPersons = persons.filter(person => person.idPersona === this.id);
      } else {
        if (this.personType === 'nrodoc') {
          this.filteredPersons = this.id !== '' ?
            persons.filter(person => person.nroDoc === this.id) :
            persons.filter(person => person.tipoPersona === 'F');
        } else {
          this.filteredPersons = this.id !== '' ?
            persons.filter(person => person.nroCUI === this.id) :
